...
def index(request):
    ...
    preservedh = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(historyIds)])
    ...
    preservedl = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(listenId)])
    listenlaters = Song.objects.filter(song_id__in=listenId).order_by(preservedl)
...
def SignupUser(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username = username).exists():
            messages.warning(request,"Username is already taken. Please Choose another one !")
            return redirect('signup')
        if len(username)<3:
            messages.warning(request,"Username should be greater then 3! ")
            return redirect('signup')
        if password1 != password2:
            messages.error(request,"Password did not match! kindly use same password in each field.")
            return redirect('signup')
