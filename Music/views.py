...
def Listenlaterfun(request):
    ...
    preserved = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(listenId)])
...
def Historyfun(request):
    ...
    preserved = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(historyIds)])
...
def Upload(request):
    ...
    if(request.method == "POST"):
        ...
        SongObj = Song(title= title,artist=artist,tags=tag,song=song, image=image,copyrights = copyrights)
        SongObj.save()
...
def profile(request):
    ...
    if request.method == "POST":
        if request.POST.get('fname') != None:
            userobj = User.objects.filter(first_name = request.user.first_name).first()
            userobj.first_name = request.POST.get('fname')
            userobj.save()
            messages.success(request,f"Your First Name sucessfully changed to {userobj.first_name}!")
            return redirect('/music/profile')

        elif request.POST.get('lname') != None:
            userobj = User.objects.filter(last_name = request.user.last_name).first()
            userobj.last_name = request.POST.get('lname')
            userobj.save()
            messages.success(request,f"Your Last Name sucessfully changed to {userobj.last_name}!")
            return redirect('/music/profile')
            ...
            userobj.save()
            messages.success(request,f"Your email sucessfully changed to {userobj.email}!")
            return redirect('/music/profile')

        elif (request.POST.get('cpass') != None) & (request.POST.get('npass1') != None) & (request.POST.get('npass2') != None):
            
            userobj = User.objects.filter(password = request.user.password).first()
            from django.contrib.auth.hashers import check_password
            # checking current password is valid or not 
            pwd_valid = check_password(request.POST.get('cpass'),userobj.password)
            ...
            if pwd_valid & (request.POST.get('napss1') == request.POST.get('npass2')):
                ...
                messages.error(request,"You enterd Wrong Password!, Kindly use your currect password")
